...
export default class AnswerExplanation extends React.PureComponent {
    ...
    componentDidMount() {
        for(var i=0;i<quesString[1].imgs.length;i++){
            if(quesString[1].imgs[i] != null) {
                document.getElementById("answerContent").insertAdjacentHTML('beforeend',quesString[1].imgs[i])
            }
       }
    }
    handleFullScreen(e) {
