...
export default class QuestionsForm extends React.PureComponent {
    ...
    componentDidMount() {
        for(var i=0;i<quesString[0].imgs.length;i++){
            if(quesString[0].imgs[i] != null) {
                document.getElementById("questionContent").insertAdjacentHTML('beforeend',quesString[0].imgs[i])
            }
       }
    }
    handleFullScreen(e) {
