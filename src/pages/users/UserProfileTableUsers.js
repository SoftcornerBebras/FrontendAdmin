...
class TableUsers extends React.PureComponent {
    ...
    async componentDidMount(){
        ...
        let gresult, Type, Date;
        try{
            gresult = await axios.get(
            baseURL+'api/usr/userProfileUsers/'+localStorage.getItem('username')+'/', {
        ...
        {
            if(gresult.data[i].created_on == gresult.data[i].modified_on)
            {
                Type = 'Created';
                Date = gresult.data[i].created_on;
            }
            else
            {
                Type = 'Modified';
                Date = gresult.data[i].modified_on;
            }
            this.setState(prevState=>({getValue:[...prevState.getValue,
            {userID:gresult.data[i].userID , date:Date , userName:gresult.data[i].username , loginID:gresult.data[i].loginID , type: Type ,
            created_on : gresult.data[i].created_on , modified_on:gresult.data[i].modified_on, created_by:gresult.data[i].created_by,
            modified_by:gresult.data[i].modified_by, emptyRows:( 3 - Math.min(3, gresult.data.length - this.state.page * 3))}]}));
        }
